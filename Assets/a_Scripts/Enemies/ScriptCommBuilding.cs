...
public class ScriptCommBuilding : MonoBehaviour 
...

{
    ...
    void Awake()
    {
        ...
        Transform temp = null;
        for (int i = 0; i < transform.childCount; i++) { Transform c = transform.GetChild(i); if (c.name == "comms") { temp = c; } }
        // Get turret
        for (int i = 0; i < temp.transform.childCount; i++) { Transform c = temp.transform.GetChild(i); if (c.name == "hinge") { hinge = c; } }
