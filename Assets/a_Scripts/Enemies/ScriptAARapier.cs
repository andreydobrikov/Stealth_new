...
public class ScriptAARapier : MonoBehaviour {
    ...
    private Vector3[] bulletXY; // also store projectile life in the Z
    ...
    void Awake()
    {
        ...
        // Get Turret hinge
        for (int i = 0; i < transform.childCount; i++) { Transform c = transform.GetChild(i); if (c.name == "turntable") { hinge = c; } }
        // Get turret
    ...
    public void Calculate_Damage(int damage)
    {
        ...
        //Damage Saved from inputs script..gets set when Bullet raycast hits something
        ...
        if (health < 0) { state = mode.destroyed; } //Debug.Log("Destroyed");
            ...
            if (locked)
            {
                if (Time.time > reloadTimer)
                {
                   ...
                   // Shoot_Bullet();
                    ...
                    reloadTimer = Time.time + Random.Range(0.1f, 0.15f);
    ...
    void Shoot_Bullet()
    {
        ...
        for (int i = 0; i < bulletXY.Length; i++)
        {
            ...
            {
                bulletXY[i].z = Time.time + 4f; // LIFE OF BULLET
                ...
                bullet.transform.eulerAngles = new Vector3(turret.eulerAngles.x + (int)Random.Range(-2f, 2f), turret.eulerAngles.y + (int)Random.Range(-2f, 2f), (int)turret.eulerAngles.z);
    ...
    public bool Track_Target()
    {
        ...
        //------------
        // RAPIERS LookAt - rotate on X axis

         //for (int i =0;  i < hinge.transform.childCount; i++)
        Transform turret = hinge.transform;
